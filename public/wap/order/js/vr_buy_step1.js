...
$(function() {
    $.ajax({
        type: "post",
        url: ApiUrl + "/Membervrbuy/buy_step2.html",
        dataType: "json",
        data: data,
        success: function(e) {
            var t = e.result;
    ...
    $("#ToBuyStep2").click(function() {
        ...
        if (!/^\d{7,11}$/.test(t)) {
            layer.open({content: '请正确输入接收手机号码',skin: 'msg',time: 2});
            return false
        }
        e.buyer_phone = t;
        ...
        $.ajax({
            type: "post",
            url: ApiUrl + "/Membervrbuy/buy_step3.html",
            data: e,
            dataType: "json",
            success: function(e) {
                checkLogin(e.login);
                ...
                if (e.result.order_id) {
                    toPay(e.result.order_sn, "membervrbuy", "pay")
                }
                return false
            }
        })
