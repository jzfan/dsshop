...
function get_list() {
    ...
    $.getJSON(ApiUrl + '/Pointprod/index.html' + window.location.search.replace('?','&'), param, function(result){
    	if(!result) {
    		result = [];
    		result.result = [];
    		result.result.goods_list = [];
    	}
        $('.loading').remove();
        curpage++;
        var html = template('home_body', result);
        $("#product_list .goods-secrch-list").append(html);
		//$('#list-items-scroll').html(template('search_items',result.result));
		if (points_min) {
...
function search_adv() {
    	...
    	$('#search_submit2').click(function(){
    		...
    		window.location.href = WapSiteUrl + '/mall/pointspro_list.html' + queryString;
    	});
    ...
    if (o == 2 && k == 'isable')
    {
        ...
        if (key == undefined || key == '')
        {
            layer.open({
                ...
                , btn: ['确定', '取消']
                , yes: function (index) {
                    addCookie('redirect_uri', '/mall/pointspro_list.html');
