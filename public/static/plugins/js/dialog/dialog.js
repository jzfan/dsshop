...
/*common2.2.js*/
DIALOGIMGDIR  = '/static/plugins/js/dialog/images/dialog';
...
//		showloading('none');
    ...
    if(action != '' && action !== null && action != 'undefined'){
        $$(formid).action = get_inajax_url(action);
    }
    $$(formid).submit();
    if(submitbtn) {
 ...
 * @returns {String}
 */
function get_inajax_url(url)
{
    //	var url = url + '&inajax=1&ajaxtarget=' + showid;
//    console.log(url.search(/inajax=1&ajaxtarget=/i));
    if (url.search(/inajax=1&ajaxtarget=/i) == - 1){
        if (url.search(/.html\?/i) > 0) {
            url = url + '&inajax=1&ajaxtarget=';
        } else {
            url = url + '?inajax=1&ajaxtarget=';
        }
    }
