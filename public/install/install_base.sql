...
CREATE TABLE `#__albumpic` (
  ...
  `aclass_id` int(10) unsigned NULL COMMENT '相册ID',
...
CREATE TABLE `#__groupbuy` (
  ...
  `groupbuy_state` tinyint(1) unsigned NOT NULL DEFAULT '20' COMMENT '抢购状态 20-正常 31-管理员关闭 32-已结束',
...
CREATE TABLE `#__waybill` (
  ...
  `waybill_image` varchar(50) NULL COMMENT '快递单模板背景图',
  ...
  `waybill_datas` varchar(2000) DEFAULT NULL COMMENT '自定义设置',
  ...
  `waybill_isdefault` tinyint(1) unsigned DEFAULT '0' COMMENT '是否默认 1:默认 2:不默认',
  `express_name` varchar(50) NOT NULL COMMENT '快递公司名称',
...
(811 ,  'swm_is_open',  '0',  '水印是否开启 0:关闭 1:开启');
