...
function goods_thumb($goods = array(), $type = '')
{
    ...
    if (in_array($upload_type['0'], array('alioss', 'cos'))) {
        ...
        if($aliendpoint_type) {
            return HTTP_TYPE.config($upload_type['0'] . '_endpoint') . '/' . ATTACH_GOODS  . '/' . $file;
        }else{
            return 'https://'.config($upload_type['0'] . '_bucket').'.'.config($upload_type['0'] . '_endpoint') . '/' . ATTACH_GOODS  . '/' . $file;
        }
    }

    $thumb_host = UPLOAD_SITE_URL . '/' . ATTACH_GOODS;
    if (!file_exists(BASE_UPLOAD_PATH . '/' . ATTACH_GOODS  . '/' . str_ireplace('.', '_' . $type . '.', $file))) {
        if(!file_exists(BASE_UPLOAD_PATH . '/' . ATTACH_GOODS  . '/' . $file)){
            return UPLOAD_SITE_URL . '/' . default_goodsimage('normal');
        }else{
...
function goods_cthumb($file, $type = '')
{
    ...
    $thumb_host = UPLOAD_SITE_URL . '/' . ATTACH_GOODS;
    if (!file_exists(BASE_UPLOAD_PATH . '/' . ATTACH_GOODS  . '/' . ($type == '' ? $file : str_ireplace('.', '_' . $type . '.', $file)))) {
        if(!file_exists(BASE_UPLOAD_PATH . '/' . ATTACH_GOODS  . '/' . $file)){
            return UPLOAD_SITE_URL . '/' . default_goodsimage('normal');
        }else{
            return $thumb_host  . '/' . $file;
...
function groupbuy_thumb($image_name = '')
{
    ...
    $file_path = ATTACH_GROUPBUY . DS . $image_name;
 ...
 * 取得订单来源类型文字输出形式
 *
 * @param array $order_from
 * @return string
 */
function get_order_from_name($order_from)
{
    return str_replace(array('1', '2', '3'), array('pc', '手机', '微信公众号'), $order_from);
}
