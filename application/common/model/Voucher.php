...
class Voucher extends Model
{
    ...
    public function getCurrentAvailableVoucher($condition = array(), $goods_total = 0)
    {
        ...
        if(!empty($voucher_list)){
            $voucher_list = ds_change_arraykey($voucher_list,'voucher_state');
        }
     ...
     * @return type
     */
    public function getCanChangeTemplateInfo($vid, $member_id)
    {
    ...
    public function getMemberVoucherList($member_id, $voucher_state, $page = null)
    {
       ...
       $list = db('voucher')->alias('v')->join('__VOUCHERTEMPLATE__ t','v.vouchertemplate_id=t.vouchertemplate_id')->where($where)->order('voucher_id desc')->paginate($page,false,['query' => request()->param()]);
        $this->page_info=$list;
        $list=$list->items();
        if (!empty($list) && is_array($list)) {
            foreach ($list as $key => $val) {
                //代金券图片
                if (empty($val['vouchertemplate_customimg']) || !file_exists(BASE_UPLOAD_PATH . DS . ATTACH_VOUCHER . DS . $val['vouchertemplate_customimg'])) {
                    $list[$key]['vouchertemplate_customimg'] = UPLOAD_SITE_URL . DS . default_goodsimage(60);
                }
                else {
                    $list[$key]['vouchertemplate_customimg'] = UPLOAD_SITE_URL . DS . ATTACH_VOUCHER . DS .$val['vouchertemplate_customimg'];
                }
                //代金券状态文字
        ...
        if (!empty($voucher_list) && is_array($voucher_list)) {
            foreach ($voucher_list as $k => $v) {
                if (!empty($v['vouchertemplate_customimg'])) {
                    $v['vouchertemplate_customimg'] = UPLOAD_SITE_URL . DS . ATTACH_VOUCHER .  DS . $v['vouchertemplate_customimg'];
                }
                else {
    ...
    public function getVouchertemplateInfo($where = array(), $field = '*', $order = '')
    {
        ...
        if (!empty($info['vouchertemplate_customimg'])) {
            $info['vouchertemplate_customimg'] = UPLOAD_SITE_URL . DS . ATTACH_VOUCHER  . DS . $info['vouchertemplate_customimg'];
        }
        else {
