...
class Waybill extends Model
{
     ...
     * @return array
     */
    public function getWaybillLists($condition, $order = '') {
        $waybill_model = model('waybill');

        $waybill_list = db('waybill')->where($condition)->order($order)->select();
        foreach ($waybill_list as $key => $value) {
            $waybill_list[$key]['is_default_text'] = $value['waybill_isdefault'] ? '是' : '否';
            $waybill_list[$key]['waybill_pixel_top'] = $value['waybill_top'] * $waybill_model::WAYBILL_PIXEL_CONSTANT;
            $waybill_list[$key]['waybill_pixel_left'] = $value['waybill_left'] * $waybill_model::WAYBILL_PIXEL_CONSTANT;
        }
        return $waybill_list;
    }
     ...
     * @return type
     */
    public function getWaybillUsableList($express_id) {
     ...
     * @return type
     */
    public function getWaybillDesignInfo($waybill_id) {
        $waybill_id = intval($waybill_id);
     ...
     * @return bool
     */
    public function editWaybillDataByID($waybill_data, $waybill_id) {
        $waybill_id = intval($waybill_id);
        if($waybill_id <= 0) {
            return false;
        }

        $item_list = $this->getWaybillItemList();
        foreach ($item_list as $key => $value) {
            $item_list[$key]['show'] = true;
        }

        $update = array();
        $update['waybill_datas'] = serialize($item_list);
     ...
     * @return type
     */
    public function saveWaybill($post) {
        $data = array();
     ...
     * @return bool|爱人ray
     */
    public function editWaybillDefault($waybill_id) {
        $waybill_id = intval($waybill_id);
        if($waybill_id <= 0) {
            return false;
        }
        ...
        //解除原默认设置
        $this->editWaybill(array('waybill_isdefault' => 0),array());

        $condition = array();
        $condition['waybill_id'] = $waybill_id;
        return $this->editwaybill(array('waybill_isdefault' => 1), $condition);
    }
}