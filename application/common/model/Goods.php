...
class Goods extends Model {
    ...
    public function delGoods($condition) {
        ...
        if (!empty($goods_list)) {
            ...
            // 删除买家收藏表数据
            model('favorites')->delFavorites(array('fav_id' => array('in', $goodsid_array)));
            // 删除商品赠品
    ...
    public function getGoodsSpecListByCommonId($common_id) {
        ...
        if (empty($spec_list)) {
            $spec_array = $this->getGoodsList(array('goods_commonid' => $common_id), 'goods_spec,goods_id,goods_image,color_id');
