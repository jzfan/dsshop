...
class Buy_1 extends Model {
    ...
    public function getMansongruleCartListByTotal($goods_total) {
        if (!config('promotion_allow') || empty($goods_total)){
    ...
    /**
     * 取得店铺可用的代金券
     * @param array $goods_total array(店铺ID=>商品总金额)
     * @return array
     */
    ...
    public function reParseVoucherList($input_voucher_list = array(), $goods_total = array(), $member_id) {
        ...
        if (is_array($voucher_list)) {
            $input_voucher_list['voucher_id'] = $voucher_list[1]['voucher_id'];
            $input_voucher_list['voucher_code'] = $voucher_list[1]['voucher_code'];
            $input_voucher_list['voucher_owner_id'] = $voucher_list[1]['voucher_owner_id'];
        } else {
    ...
    public function getFreightDescList($goods_total) {
        ...
        $limit_price = floatval(config('free_price'));
        if ($limit_price == 0 || $limit_price > $goods_total) {
            //需要计算运费
            $need_calc_sid_array = '';
        } else {
            //返回免运费金额下限
            $cancel_calc_sid_array['free_price'] = $limit_price;
            $cancel_calc_sid_array['desc'] = sprintf('满%s免运费', $limit_price);
        }
     ...
     */
    public function getFreightList($buy_list = array(), $free_freight_sid_list) {
        //定义返回数组
        $return = array();
        //先将免运费的店铺运费置0(格式:店铺ID=>0)
        $freight_list = 0;
        if (!empty($free_freight_sid_list)) {
            $freight_list = 0;
        }
        ...
        foreach ($buy_list as $key => $goods_info) {
            ...
            if (!intval($goods_info['transport_id'])) {
