...
class Buy extends Model
{
    ...
    public function getBuyStep1Data($member_id, $data)
    {
        ...
        list($need_calc_sid_list, $cancel_calc_sid_list) = $this->_logic_buy_1->getFreightDescList($goods_total);
        ...
        //$cancel_calc_sid_list 不为空则表示免运费
        $freight_list = $this->_logic_buy_1->getFreightList($goods_list, $cancel_calc_sid_list);
...
//        }
    ...
    private function _createOrderStep2()
    {
        ...
        if ($post['ifcart']) {
            ...
            if (isset($post['goods_type'])) {
                $extra['goods_type'] = $post['goods_type'];
            }
            //商品信息[得到最新商品属性及促销信息]
            $goods_info = $this->_logic_buy_1->getGoodsOnlineInfo($goods_id, intval($quantity),$extra);
            if (empty($goods_info)) {
    ...
    private function _createOrderStep3()
    {
        ...
        //计算每个店铺运费
        list($need_calc_sid_list, $cancel_calc_sid_list) = $this->_logic_buy_1->getFreightDescList($final_goods_total);
        //$cancel_calc_sid_list 不为空则表示免运费
        $freight_list = $this->_logic_buy_1->getFreightList($goods_list, $cancel_calc_sid_list);
        $freight_total = $this->_logic_buy_1->calcFreight($freight_list, $input_city_id);
        if(empty($freight_total)){
            //DSSHOP暂时屏蔽
    ...
    private function _createOrderStep6()
    {
        ...
        if ($ifgroupbuy) {
            foreach ($cart_list as $goods_list) {
                if ($goods_list['ifgroupbuy'] && $goods_list['groupbuy_id']) {
                    $groupbuy_info = array();
