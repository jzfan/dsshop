...
class Goods extends BaseGoods {
    ...
    public function index() {
       ...
       // halt($goods_info);
        ...
        //推荐商品 
        $goods_commend_list = $goods_model->getGoodsOnlineList(array( 'goods_commend' => 1), 'goods_id,goods_name,goods_advword,goods_image,goods_price', 0, '', 5, 'goods_commonid');
        ...
        if (empty($hot_sales_list)) {
            ...
            $this->assign('hot_sales', $hot_sales_list);
        ...
        if (empty($hot_collect_list)) {
            ...
            $this->assign('hot_collect', $hot_collect_list);
    ...
    public function salelog() {
        ...
        $this->assign('order_type', array(2 => lang('ds_groupbuy_flag'), 3 => lang('ds_xianshi_flag'), '4' => lang('ds_xianshi_suit')));
    ...
    private function checkConsultAble() {
        ...
        //检查是否可以评论
        $consult_able = true;
        if ((!config('guest_comment') && !session('member_id') ) || (session('member_id') > 0 && $member_info['is_allowtalk'] == 0)) {
            $consult_able = false;
        }
        return $consult_able;
    }
...
//        $js = "ajax_form('arrival_notice', '" . $title . "', '" . url('Goods/arrival_notice_succ', ['type' => $type]) . "', 480);";
    ...
    function getGoodsDetail_bak($goods_id) {
        ...
        // 查询所有规格商品
        $spec_array = db('goods')->where('goods_commonid', $goods_info1['goods_commonid'])->field('goods_spec,goods_id,goods_image,color_id')->select();
