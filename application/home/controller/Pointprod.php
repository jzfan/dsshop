...
class Pointprod extends BasePointShop {
    ...
    public function _initialize() {
        ...
        if (config('pointprod_isuse') != 1) {
            ds_show_dialog(lang('pointprod_unavailable'), url('Index/index'), 'error');
        }
    ...
    public function pinfo() {
        $pid = intval(input('id'));
        if (!$pid) {
            ds_show_dialog(lang('param_error'), url('Pointshop/index'), 'error');
        }
        $pointprod_model = model('pointprod');
        //查询兑换礼品详细
        $prodinfo = $pointprod_model->getOnlinePointProdInfo(array('pgoods_id' => $pid));
        if (empty($prodinfo)) {
            ds_show_dialog(lang('pointprod_record_error'), url('Pointprod/plist'), 'error');
        }
