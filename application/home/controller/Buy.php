...
class Buy extends BaseMember {
    ...
    public function buy_step1() {
        ...
        $result = $buy_logic->buyStep1($_POST['cart_id'], $ifcart, session('member_id'));
        ...
        $this->assign('goods_total', $result['goods_total']);

        //取得店铺优惠 - 满即送(赠品列表，店铺满送规则列表)
        $this->assign('premiums_list', $result['premiums_list']);
        $this->assign('mansong_rule_list', $result['mansong_rule_list']);

        //返回店铺可用的代金券
        $this->assign('voucher_list', $result['voucher_list']);
