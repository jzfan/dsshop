...
class Membermessage extends BaseMember
{
    ...
    public function savemsg()
    {
        ...
        $isallowsend = $this->allowSendMessage(session('member_id'));
        if (!$isallowsend) {
            ds_show_dialog(lang('home_message_noallowsend'));
        }
        $data=[
        ...
        if (!$message_validate->scene('savemsg')->check($data)) {
            ds_show_dialog($message_validate->getError());
        }
        ...
        if (!empty($member_list)) {
            ...
            foreach ($member_list as $k => $v) {
                ...
                $return = $message_model->addMessage($insert_arr);
            }
        }
        else {
            ds_show_dialog(lang('home_message_receiver_error'));
        }
        if (input('param.type') == 'sns_board') {
            ...
            ds_show_dialog(lang('home_message_send_success'), '', 'succ', $js);
        }
        else {
            ds_show_dialog(lang('home_message_send_success'), url('Membermessage/privatemsg'), 'succ');
        }
    }
    ...
    public function savereply()
    {
        ...
        if (request()->isPost()) {
            ...
            if (empty(input('post.msg_content'))) {
                if (input('param.inajax') == 1) {
                    ds_show_dialog(lang('error'));
                }
                else {
                    $this->error(lang('error') . lang('home_message_reply_content_null'));
                }
            }
            $message_model = model('message');
            //查询站内信
            ...
            if (empty($message_info)) {
                if (input('param.inajax') == 1) {
                    ds_show_dialog(lang('home_message_no_record'));
                }
                else {
            ...
            if (input('param.inajax') == 1) {
                ds_show_dialog(lang('home_message_reply_command_wrong'));
            }
            else {
