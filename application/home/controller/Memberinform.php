...
class Memberinform extends BaseMember {
    ...
    public function index() {
        ...
        $condition = array();
        if (input('param.select_inform_state')){
    ...
    public function inform_save() {
        ...
        //检查是否当前正在举报
        if ($inform_model->isProcessOfInform($goods_id)) {
            ds_show_dialog(lang('inform_handling'));
        }
        //处理用户输入的数据
        $input = array();
        ...
        if (!$inform_validate->scene('inform_save')->check($data)) {
            ds_show_dialog($inform_validate->getError());
        }

        //保存
        if ($inform_model->addInform($input)) {
            ds_show_dialog(lang('inform_success'), url('Memberinform/index'), 'succ');
        } else {
            ds_show_dialog(lang('inform_fail'), url('Memberinform/index'), 'error');
        }
    }
