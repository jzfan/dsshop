...
class Memberfavorites extends BaseMember
{
    ...
    public function fglist()
    {
        ...
        $this->assign('collection_goods_list', $collection_goods_list);
    ...
    public function delfavorites()
    {
        ...
        if (!empty($fav_arr) && is_array($fav_arr)) {
            $favorites_list = $favorites_model->getFavoritesList(array('fav_id' => array('in', $fav_arr), 'member_id' => session('member_id')));
            if (!empty($favorites_list) && is_array($favorites_list)) {
                $fav_arr = array();
                ...
                if (!empty($fav_arr) && $result) {
                    ...
                    if ($_GET){
                        ds_show_dialog(lang('ds_common_del_succ'),url('Memberfavorites/fglist'),'succ');
                    }else{
